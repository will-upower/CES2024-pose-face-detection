...
# from .utils.camera_mmap import Camera_mmap
...
# names of the mmap shared memory files
in_file_mmap    = "v4h2_to_orin.dat"
out_file_mmap   = "orin_to_v4h2.dat"

# dimensions of image being written and read from mmap buffer
MMAP_BUFF_HEIGHT    = 1280
MMAP_BUFF_WIDTH     = 720
# number of bytes per pixel in yuyv image conversion to rgb88
NUM_BYTE_PER_PIXEL = 3



def test_write_image_to_mmap(frame):
    file_size = frame.nbytes
    frame_data = frame.tobytes()

    with open(out_file_mmap, 'wb') as f:
        f.seek(file_size - 1)
        f.write(b'\0')  # Extend the file to the desired size
        f.seek(0)
        f.write(frame_data)
        f.close
...
"""
# Takes image frame and writes to mmap file buffer
# mmap image should be a 1280 x 720 x 3 bytes buffer (3 bpp for RGB)
def write_image_to_mmap(frame):
    # Create a memory-mapped file
    file_size = frame.nbytes  # Size of the image frame in bytes
    with open(out_file_mmap, 'wb') as f:
        f.seek(file_size - 1)
        f.write(b'\0')  # Extend the file to the desired size

    # Map the file into memory
    with open(out_file_mmap, 'w+b') as f:
        mmapped_file = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_WRITE)

    # Convert the OpenCV frame to a NumPy array
    image_array = np.asarray(frame)

    # Write the NumPy array to the memory-mapped file
    mmapped_file[:file_size] = image_array.tobytes()
...
class GUI:
    ...
    def __init__(self, args, db, pose_estimator, face_detector,
        ...
        mmap_obj = Camera(args.cam_dist, in_file_mmap, MMAP_BUFF_HEIGHT, MMAP_BUFF_WIDTH, args.width, args.height)
        mmap_obj.start()
        yres, xres = mmap_obj.res
    ...
    def __call__(self):
        ...
        while not self.end:
            ...
            ret, frame, frame_rgb, pose_frame = self.read()
    ...
    def render_overlay(self, frame, people, tracks, heatmap):
        ...
        # print(str(disFrame.shape))
        # print(str(disFrame.size))

        ### Image out ###
        cv2.imshow(self.name, disFrame)
        # test_write_image_to_mmap(disFrame)
